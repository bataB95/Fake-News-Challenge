...
size=len(data)
trainset=np.array(data[1200:])
np.random.shuffle(trainset)
trainset=dataWrapper(trainset)
testset=dataWrapper(data[:1200])
data=None
#network parameters
learning_rate=0.001
training_iters=5*trainset.size
#training_iters=1308
batch_size=128
display_step=10
...
with tf.device('/gpu:'+str(sys.argv[1])):
    sess=tf.Session(config=config)
    sess.run(init)
    step=1
    #saver.restore(sess, "/tmp/model_related_1.0-10")
    ...
    while step*batch_size<training_iters:
        ...
        if step%display_step==0:
            ...
            print("Iter " + str(step*batch_size) + ", Minibatch Loss= " + \
                  ...
                  "{:.5f}".format(acc))
            save_path = saver.save(sess, "/uac/y15/kcli5/FNC-1/tmp/model_related_1.0",global_step=step)
            print("Model saved in file: %s" % save_path)
        step+=1
    print("Optimization Finished!")
    #save_path = saver.save(sess, "/tmp/model_related_1.0.ckpt")
