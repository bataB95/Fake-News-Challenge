...
for sample in data[0:test_size]:
	if sample[1]==0:
		test_set.append([list(sample[0]),0])
	else:
		test_set.append([list(sample[0]),1])
for sample in data[test_size:size-1]:
	train_set.append([list(sample[0]),sample[1]])
print(len(train_set[0][0]))
feature_columns=[tf.contrib.layers.real_valued_column("",dimension=50)]
classifier=tf.contrib.learn.DNNClassifier(feature_columns=feature_columns,hidden_units=[100],n_classes=2,activation_fn=tf.nn.relu)
def get_train_set():
...
def get_test_set():
	...
	return x,y
classifier.fit(input_fn=get_train_set,steps=10000)
